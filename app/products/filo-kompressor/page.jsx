...
const FiloKompresor = () => {
  ...
  return (
    <section>
      <title>Filo Kompressor-LALETECH MMC | LALETECH MMC | Mühəndislik Şirkəti</title>
      ...
      {loading ? (
        ...
        <div className='all-products' style={{ marginBottom: '100px' }}>
          <Grid className='grid' container spacing={{ xs: 12, sm: 6, md: 4, lg: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
            {filoProducts &&
              filoProducts.map((filoProduct) => (
                <Grid key={filoProduct._id} item lg={3}>
                  {/* Filo Product Card */}
                  <div class="card">
      <div class="card-image-container">
      <Image
              className='prod-img'
              src={filoProduct.imageURL}
              alt='Packing'
              width={400}
              height={400}
            />
      </div>
      <p class="card-title">{filoProduct.name}</p>

      <Link href={filoProduct.link} class="card-btn">
